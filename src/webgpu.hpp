...
private:
    ...
    wgpu::TextureView textureViewY = nullptr;
    wgpu::TextureView textureViewU = nullptr;
...
//		SDL_Log("out_pix_fmt: %s\n", av_get_pix_fmt_name(pix_fmt));
	...
	wgpu::ComputeState load_shader(ShaderType type)
	{
...
//            stage_desc.entryPoint = "main";
    ...
    wgpu::ShaderModule create_shader(const char* wgsl_source)
    {
    ...
    bool init_pipeline()
    {
        ...
        auto vertex_shader_module = create_shader(__shader_vert_wgsl);
//        auto vertex_shader_desc = load_shader(VERT);
//        if (!vertex_shader_desc.module) vertex_shader_desc = ImGui_ImplWGPU_CreateShaderModuleSPIRV(__shader_vert_spirv, sizeof(__shader_vert_spirv));
        ...
        auto frag_shader_module = create_shader(__test_frag_wgsl);
//        auto frag_shader_desc = load_shader(NV12_EXT_FRAG);
...
//            .entryPoint = frag_shader_desc.entryPoint,
            .targetCount = 1,
            .targets = &color_state
        };
...
//        textureDesc.usage = wgpu::TextureUsage::TextureBinding;
        ...
        texture = sharedMemory.CreateTexture(&textureDesc);
        ...
        textureViewY = texture.CreateView(&plane0Desc);
        ...
        if (va_desc.num_layers > 1) {
            ...
            textureViewU = texture.CreateView(&plane1Desc);
        }
    ...
    void render(AppSub sub) {
        ...
        sharedMemory.BeginAccess(texture, &beginDesc);
...
/*
    bool is_supported_drm() {
        wgpu::DawnDrmFormatProperties drmProperties{};
        drmProperties.drmFormat = 0x30313050; // Pass the raw P010 format index to ask the driver

        wgpu::AdapterInfo info{};
        info.nextInChain = &drmProperties;
        adapter.GetInfo(&info);

        // Check if Dawn natively exposes texture bindings for this specific format
        if (drmProperties.features & wgpu::DawnDrmFormatCapabilityFeature::TextureBinding) {
            // The format is fully supported by this custom Dawn snapshot build!
        } else {
            // Driver explicitly returned zero capability support, confirming you must convert to NV12
        }
    }*/
};